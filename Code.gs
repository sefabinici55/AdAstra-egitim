// ============================================================
// Code.gs — CRM Pipeline Dashboard (Google Apps Script Web App)
// Dağıtım: Apps Script → Dağıt → Web uygulaması
// ============================================================
...
// Kolon indeksleri
var C = { TEMSILCI:2, BOLGE:3, KAYNAK:5, ASAMA:6, TUTAR:7, OLASILIK:8, ACIK_GUN:10 };
var OPEN = ['Qualification','Proposal','Negotiation'];

// ── Web app giriş noktası ─────────────────────────────────
function doGet() {
  return HtmlService.createHtmlOutputFromFile('index')
    .setTitle('CRM Pipeline Dashboard')
    .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
}

// ── Tüm dashboard verisini tek seferde döndür ─────────────
...
// ── KPI hesaplamaları ─────────────────────────────────────
function _kpis() {
  var tp=0, wp=0, won=0, cnt=0, days=0;
  DATA_ROWS.forEach(function(r) {
    var s=r[C.ASAMA], t=r[C.TUTAR], o=r[C.OLASILIK]/100, d=r[C.ACIK_GUN];
    if (OPEN.indexOf(s) > -1) { tp+=t; wp+=t*o; cnt++; days+=d; }
    if (s==='Closed Won') won+=t;
  });
  return { totalPipeline:tp, weightedPipeline:wp, wonRevenue:won,
           openCount:cnt, avgOpenDays: cnt>0 ? Math.round(days/cnt) : 0 };
}

// ── Huni: her aşama için sayı + tutar ────────────────────
function _funnel() {
  var stages=['Qualification','Proposal','Negotiation','Closed Won','Closed Lost'];
  var agg={};
  stages.forEach(function(s){ agg[s]={count:0,tutar:0}; });
  DATA_ROWS.forEach(function(r){
    var s=r[C.ASAMA];
    if(agg[s]){ agg[s].count++; agg[s].tutar+=r[C.TUTAR]; }
  });
  return stages.map(function(s){ return {stage:s, count:agg[s].count, tutar:agg[s].tutar}; });
}

// ── Bölge: kazanılan ciro ─────────────────────────────────
function _bolge() {
  var agg={};
  DATA_ROWS.forEach(function(r){
    var b=r[C.BOLGE];
    if(!agg[b]) agg[b]=0;
    if(r[C.ASAMA]==='Closed Won') agg[b]+=r[C.TUTAR];
  });
  return Object.keys(agg).map(function(k){ return {bolge:k, tutar:agg[k]}; })
               .sort(function(a,b){ return b.tutar-a.tutar; });
}

// ── Temsilci: kazanılan ciro + kazanma oranı ─────────────
function _temsilci() {
  var agg={};
  DATA_ROWS.forEach(function(r){
    var t=r[C.TEMSILCI];
    ...
    agg[t].total++;
    if(r[C.ASAMA]==='Closed Won'){ agg[t].won++; agg[t].wonTutar+=r[C.TUTAR]; }
  });
  return Object.keys(agg).map(function(t){
    var v=agg[t];
    return { temsilci:t, wonTutar:v.wonTutar,
             kazanmaOrani: v.total>0 ? Math.round(v.won/v.total*100) : 0,
             won:v.won, total:v.total };
  }).sort(function(a,b){ return b.wonTutar-a.wonTutar; });
}
...
function _kaynak() {
  ...
  DATA_ROWS.forEach(function(r){
    ...
    agg[k].count++; agg[k].tutar+=r[C.TUTAR];
  });
  return Object.keys(agg).map(function(k){
    return {kaynak:k, count:agg[k].count, tutar:agg[k].tutar};
